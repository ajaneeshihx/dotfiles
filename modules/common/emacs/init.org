...
* mu4e - Modus Light Theme

#+begin_src emacs-lisp :tangle yes

(defun my/apply-buffer-local-theme ()
  "Apply the light theme to mu4e buffers only."
  (when (derived-mode-p 'mu4e-view-mode 'mu4e-compose-mode)
    ;; Ensure we're working with buffer-local faces
    (buffer-face-mode 1)
    
    ;; Set buffer-local face remapping for key faces
    (face-remap-add-relative 'default :background "#ffffff" :foreground "#000000")
    (face-remap-add-relative 'mu4e-header-face :foreground "#333333")
    (face-remap-add-relative 'mu4e-unread-face :foreground "#2020cc" :weight 'bold)
    (face-remap-add-relative 'mu4e-highlight-face :background "#f0f0f0" :foreground "#000000")
    (face-remap-add-relative 'mu4e-header-highlight-face :background "#e0e0e0" :foreground "#000000")
    (face-remap-add-relative 'fringe :background "#f0f0f0")
    
    ;; Adjust more faces as needed
    (face-remap-add-relative 'mu4e-cited-1-face :foreground "#336699")
    (face-remap-add-relative 'mu4e-cited-2-face :foreground "#558866")
    (face-remap-add-relative 'mu4e-cited-3-face :foreground "#884466")
    
    ;; Text rendering adjustments
    (face-remap-add-relative 'variable-pitch :family "Source Sans Pro" :height 110)
    
    ;; Set light background for the mode line in this buffer
    (face-remap-add-relative 'mode-line :background "#e8e8e8" :foreground "#303030")
    (face-remap-add-relative 'mode-line-inactive :background "#d8d8d8" :foreground "#505050")))

;; Hook into all mu4e modes to apply buffer-local theming
(add-hook 'mu4e-main-mode-hook 'my/apply-buffer-local-theme)
(add-hook 'mu4e-headers-mode-hook 'my/apply-buffer-local-theme)
(add-hook 'mu4e-view-mode-hook 'my/apply-buffer-local-theme)
(add-hook 'mu4e-compose-mode-hook 'my/apply-buffer-local-theme)
